...
def main() -> None:
    ...
    prompt_l3 = textwrap.dedent(f"""
        以下の情報をもとに、西暦 {year} 年を舞台にした **{pattern_name}（{pattern_axis}）** のショートストーリーを書いてください。

        ## 制約（厳守）
        - 文字数：900〜1000字
        - 文体：吉川英治や宮沢賢治のように流麗で情緒的  
          *ただし難読語は避け、中学生が読める水準に抑える*
        - 感情・心理描写を深く掘り下げ、**詩的比喩を最低 2 回** 用いる
        - 主人公が困難→意外な展開→成長→ハッピーエンド
        - 会話はライトノベル風に親しみやすく
        - 全角カギ括弧「」を会話に使用、ふりがなは ( ) で
